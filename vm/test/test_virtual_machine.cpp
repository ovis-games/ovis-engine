...
TEST_CASE("Test virtual machine", "[ovis][vm][VirtualMachine]") {
  ...
  SECTION("Insert instructions") {
    ...
    REQUIRE(memcmp(instructions.data(), instruction_pointer, sizeof(Instruction) * instructions.size()) == 0);
  }
  ...
  SECTION("Insert constants") {
    ...
    REQUIRE(constant_pointer[0].as<double>() == 16.0);
    REQUIRE(*constant_pointer[1].as<std::shared_ptr<double>>() == 32.0);
    REQUIRE(constant_pointer[1].as<std::shared_ptr<double>>().use_count() == 2);
  }
}
