...
class ScriptFunctionParser {
 ...
 private:
  ...
  void ParsePush(const json& value_definiion, const std::string& path, std::shared_ptr<vm::Type> required_type = nullptr, std::size_t stack_frame_offset = 0);
  std::optional<std::size_t> GetOutputVariablePosition(std::string_view name, std::shared_ptr<vm::Type> type, const std::string& path);
