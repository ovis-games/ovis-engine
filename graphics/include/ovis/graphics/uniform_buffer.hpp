...
class UniformBuffer : public GraphicsResource {
 ...
 public:
  ...
  inline void SetUniform(const std::string& name, T&& value) {
    ...
    assert(m_uniform_descriptions[it_uniform->second].type == OpenGLType<std::remove_reference_t<T>>);
  ...
  inline void SetTexture(const std::string& sampler_name, Texture2D* texture) {
    assert(m_uniform_indices.find(sampler_name) != m_uniform_indices.end());
    assert(m_uniform_indices[sampler_name] < m_uniform_descriptions.size());
    assert(m_uniform_descriptions[m_uniform_indices[sampler_name]].type == GL_SAMPLER_2D);
  ...
  inline void SetTexture(const std::string& sampler_name, Cubemap* texture) {
    assert(m_uniform_indices.find(sampler_name) != m_uniform_indices.end());
    assert(m_uniform_indices[sampler_name] < m_uniform_descriptions.size());
    assert(m_uniform_descriptions[m_uniform_indices[sampler_name]].type == GL_SAMPLER_CUBE);
    SetTexture(m_uniform_descriptions[m_uniform_indices[sampler_name]].base_texture_unit, texture);
  }

  inline void SetTexture(const std::size_t sampler_index, Texture* texture) {
    assert(sampler_index < m_textures.size());
 ...
 private:
  ...
  inline void* GetUniformPointer(std::size_t uniform_index) {
    assert(uniform_index < m_uniform_descriptions.size());
